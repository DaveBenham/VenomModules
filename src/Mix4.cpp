...
struct Mix4 : Module {
  ...
  Mix4() {
    ...
    configSwitch(MODE_PARAM, 0.f, 2.f, 0.f, "Level Mode", {"Audio dB", "CV%", "CV x10"});
    configSwitch(CLIP_PARAM, 0.f, 2.f, 0.f, "Clipping", {"Off", "Soft audio clipping", "Hard CV clipping"});
  ...
  void process(const ProcessArgs& args) override {
    if( static_cast<int>(params[MODE_PARAM].getValue()) != mode ||
      ...
      for (int i=0; i<4; i++) {
        ...
        q->unit = mode == 0 ? " dB" : !connected[i] ? " V" : mode == 1 ? "%" : "x";
        q->displayBase = mode == 0 ? -10.f : 0.f;
        q->displayMultiplier = mode == 0 ? 20.f : (mode == 1 && connected[i]) ? 100.f : 10.f;
        q->displayOffset = mode == 0 ? 0.f : (mode == 1 && connected[i]) ? -100.f : -10.f;
      }
      q = paramQuantities[MIX_LEVEL_PARAM];
      q->unit = mode == 0 ? " dB" : mode == 1 ? "%" : "x";
      q->displayBase = mode == 0 ? -10.f : 0.f;
      q->displayMultiplier = mode == 0 ? 20.f : mode == 1 ? 100.f : 10.f;
      q->displayOffset = mode == 0 ? 0.f : mode == 1 ? -100.f : -10.f;
      q->defaultValue = mode == 0 ? 1.f : mode == 1 ? 2.f : 1.1f;
      normal = mode == 0 ? 0.f : mode == 1 ? 10.f : 1.f;
      scale = mode == 2 ? 10.f : 1.f;
      offset = mode == 0 ? 0.f : -1.f;
    }

    int channels = std::max({1, inputs[INPUTS].getChannels(), inputs[INPUTS+1].getChannels(), inputs[INPUTS+2].getChannels(), inputs[INPUTS+3].getChannels()});
    simd::float_4 out = simd::float_4::zero();
    for (int c=0; c<channels; c+=4){
      out = inputs[INPUTS+0].getNormalPolyVoltageSimd<simd::float_4>(normal, c) * (params[LEVEL_PARAMS+0].getValue()+offset)*scale
